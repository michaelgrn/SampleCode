...
/*NOTE: This is an incomplete test you are expected to expand on this
 * to ensure your code is correct!
 */
int main(int argc, char *argv[])
{
	...
	//char* newString = buddyMalloc(1000);
	//printf("%p location of new string\n", newString);
	//buddyFree(newString);
	...
	for (i=0; i < count; i++) {
		...
		if (x[loc].ptr) {
			...
			//x[loc].ptr = (char *) buddyMalloc(size*sizeof(char));
			size = random() % MAX_REQUEST + 1; // how big a request
			
			//x[loc].ptr = (char *) buddyRealloc((void*)x[loc].ptr, size*sizeof(char));
			/*
			if(i % 2 == 1){
			    fprintf(stderr, "realloc\n");
			    x[loc].ptr = (char *) buddyRealloc(x[loc].ptr, size*sizeof(char)*4);
			}else{
	...
	//char* newString2 = buddyMalloc(4000);
	//scanf("%s", newString);
	//printf("%s", newString);
	/*
	if (argc < 2) {
