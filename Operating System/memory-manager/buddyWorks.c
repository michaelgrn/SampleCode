...
const void addNode(struct block_header *toAdd);
const void removeSpecificNode(struct block_header *buddy);
...
static int buddy_init(size_t size) {
    void* brk;
    int logSize = 0;
    int x = 0;
    size_t newSize = 0;
...
void *malloc(size_t size)
{
  ...
  if(currentBlock == -1){
    ...
    removedBlock = removeNode(currentBlock);
  //  printf("%p\n", removedBlock);
  }
 ...
 //   printf("split\n");
	removedBlock = split(removedBlock);
